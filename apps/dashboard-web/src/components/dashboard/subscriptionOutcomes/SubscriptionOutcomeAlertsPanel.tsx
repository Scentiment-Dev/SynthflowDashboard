...
export default function SubscriptionOutcomeAlertsPanel({
  ...
  if (alerts.length === 0) {
    return (
      ...
      >
        All Stay.ai-controlled subscription outcomes are clear and confirmed for the current
        contract response.
      </section>
    );
  }
        ...
        {alerts.map((alert) => {
          ...
          return (
            <li
              ...
              className={`flex items-start gap-3 rounded-2xl border p-3 ${LEVEL_STYLES[alert.level]}`}
            >
